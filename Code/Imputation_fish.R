# Work through code and make appropriate for Act data
...
# name in the tree
# the GE score
load("FishAllSpecies.Rdata")

# load up the taxanomic information
tax <- load_taxa()

master.list$family <- NA
master.list$order <- NA
master.list$scientificName <- gsub("_", " ", master.list$Species)
# let's add the family group to the master.list

for (i in 1:length(master.list$Species)) {
  temp.species <- master.list$scientificName[i]
  ref <- as.numeric(match(temp.species, tax$Species))
  master.list$family[i] <- tax$Family[ref]
  master.list$order[i] <- tax$Order[ref]
}

# now save down the updated df in case it's useful later
save(master.list, file = "Data/Updated_masterlist.Rdata")

ch.species <- data.frame(Family = master.list$family, RL = master.list$RLScore, Species = master.list$Species, Tree.spp = master.list$TreeSpp, 
                          GE = NA)


ch.species$Family <- as.character(ch.species$Family)
ch.species$Species <- as.character(ch.species$Species)
ch.species$Tree.spp <- as.character(ch.species$Tree.spp)
ch.species$RL <- as.character(ch.species$RL)

# last thing: let's add in the GE values!

source("../../ResearchProject/Code/EDGE_functions.R")

ch.species <- calculate.GE(ch.species)

# now we have the 'species' df that we needed!

# make sure it's all in thr right foramt
ch.species$Tree.spp <- gsub(" ", "_", ch.species$Tree.spp)

save(ch.species, file = "Data/ChSpecies_df.Rdata")
load("Data/ChSpecies_df.Rdata")
...
#phy <- crocs
phy <- read.nexus(file = "Data/output.nex") # 10k trees
...
# this creates list of backup trees 
phy.spare <- phy[sample(c(1:length(phy))[-tree.samp],100,replace = F)]

# now create the lists of the trees 
phy <- phy[tree.samp]
...
for(i in 1:length(phy)){
  if(!class(phy[[i]]) == "phylo"){
    phy[[i]] <- as(phy[[i]], "phylo")
  }
...
head(Species)
save(Species, file = "ChSpecies.Rdata")
...
phy.block.1000 <- phy
save(phy.block.1000, file = "1000_Chond_phylos.Rdata")


#plot(phy.block.1000[[1]])
